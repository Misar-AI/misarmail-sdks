...
Official TypeScript SDK for the [MisarMail](https://misarmail.com) API — transactional
...
```ts
import { MisarMailClient, PlanLimitError } from "@misarmail/sdk";
...
const contacts = await mail.contacts.list({ page: 1, limit: 50 });
...
console.log(plan.sending, plan.usage);   // usage is an array, one entry per metered feature

try {
  await mail.campaigns.create({ name: "Blast", subject: "Hi", fromName: "You", fromEmail: "you@yourdomain.com" });
} catch (err) {
  if (err instanceof PlanLimitError) {
    console.error(`${err.feature} exhausted on ${err.plan}`, err.upgradeUrl);
  }
}
```
...
for await (const progress of mail.streaming.campaignSend(campaign.id)) {
